...
class CorrectnessTest extends FunSpec with Matchers {
  ...
  val calculators: List[GravityCalculator] = List(
    ...
    new StandardGravityCalculator(),
    new GravityCalculatorV2()
  )
  describe("Orbits should be correct for ") {
    for (calc <- calculators) {
      describe("calculator "+calc) {
        it("earth") {
          val space: Space = new Space(new DrawingSurface, Days(1), new NaiveGravityCalculator())
